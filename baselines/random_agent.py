...
DECEPTION_TYPES = [
    "civilian_military", "commercial_weapons",
    "construction_fortification", "logistics_supply", "research_weapons",
]
...
def random_action(task_id: int) -> dict:
    ...
    if task_id == 2:
        return {"action": action, "threat_level": random.randint(1, 10)}
        ...
        return {
            "action": action,
            "cover_story_identified": "unknown cover story",
            "deception_type": random.choice(DECEPTION_TYPES),
            "reasoning": "Random selection with no analysis applied.",
        }
...
def run_episode(task_id: int, seed: int) -> float:
    ...
    print(f"[Random Agent] Task {task_id} score: {score:.3f}", flush=True)
    return score
...
def main():
    ...
    print("  Task 4: ~0.15 (3 actions + 5 deception types + cover story)")
    print("=" * 50)
