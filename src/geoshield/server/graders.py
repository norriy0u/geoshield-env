...
def grade_task1(action: GeoShieldAction, case: Dict[str, Any]) -> GeoReward:
    ...
    if action.action == gold:
        return GeoReward(
            score=_clamp(0.99),
            feedback=f"Correct! '{action.action}' matches gold.",
            breakdown={"action_score": 0.99},
        )

    # Partial credit for hard/medium wrong answers — reward signal matters
    partial = {"easy": 0.01, "medium": 0.20, "hard": 0.35}.get(difficulty, 0.01)
    return GeoReward(
        score=_clamp(partial),
        feedback=f"Incorrect. Expected '{gold}', got '{action.action}'. ({difficulty} case)",
        breakdown={"action_score": partial},
    )
